...
public class Analisador {
    ...
    public String saldoMedioPorProfissao(){
        ...
        var profissoes = this.transacoes.stream().map(Transaction::occupation).distinct().toList();
        //Cria a String de saída
        String saida = "--- MÉDIA DE SALDO POR PROFISSÃO ---\n";


        for (var profissao : profissoes){//Itera em cada profissão

            //separa as transações por profissão
            Stream<Transaction> transacoes = this.transacoes.stream().filter(t -> t.occupation().equals(profissao));

            //tira a média dos saldos (balance) de cada profissão
            double media = transacoes.map(Transaction::balance)
                                     .collect(Collectors.averagingDouble(BigDecimal::doubleValue))
                                     .doubleValue();

            //adiciona os textos na saída para cada profissão
            saida += "%s: R$ %.2f\n".formatted(profissao, media);
        }
        return saida;
    }
